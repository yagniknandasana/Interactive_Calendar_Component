...
const WallCalendar = () => {
  ...
  // Notes handlers
  const currentDateNote = dateRange.start && !dateRange.end ? getDateNote(dateRange.start) : "";
  const currentRangeNote =
    dateRange.start && dateRange.end ? getRangeNote(dateRange.start, dateRange.end) : "";

  const handleDateNoteChange = useCallback(
    (text: string) => {
      if (dateRange.start && !dateRange.end) {
        setDateNote(dateRange.start, text);
      }
    },
    [dateRange, setDateNote]
  );

  const handleRangeNoteChange = useCallback(
    (text: string) => {
      if (dateRange.start && dateRange.end) {
        setRangeNote(dateRange.start, dateRange.end, text);
      }
    },
    [dateRange, setRangeNote]
  );
            ...
            <div className="flex flex-col md:flex-row gap-5">
              ...
              <div className="md:w-[33%] w-full md:border-r md:border-calendar-line/60 md:pr-5">
                <NotesPanel
                  ...
                  dateNote={currentDateNote}
                  rangeNote={currentRangeNote}
                  onGeneralChange={setGeneralNote}
                  onDateChange={handleDateNoteChange}
                  onRangeChange={handleRangeNoteChange}
                />
              </div>
              {/* Calendar grid */}
              <div className="md:w-[67%] w-full">
                ...
                <CalendarGrid
                  ...
                  hasRangeNote={getDateRangeIndicators}
                />
              </div>
            </div>
